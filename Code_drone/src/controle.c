...
void i2c(void) {
	...
	char config[2];
	/* 0x2C (soit ',' et 44), sélection du registre de taux de bande passante */
	config[0] = 0x2C;
	...
	/* 0x2D (soit '-' et 45), configuration du registre de controle de puissance */
	config[0] = 0x2D;
	...
	/* 0x31 (soit '1' et 49), sélectionnez le format de données */
	config[0] = 0x31;
	/* 0x08 (soit '\b' et 8), autotest désactivé, interface à 4 fils, résolution complète, plage = +/- 2g (0x08) */
	config[1] = 0x08;
	/* Ecrire la configuration dans le flux de taille 2 */
